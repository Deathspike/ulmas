...
export class SectionCache {
  constructor(sectionId: string) {
    this.fullPath = path.join(app.settings.paths.cache, `series.${sectionId}.json`);
  }

  async loadAsync() {
    const sectionJson = await fs.promises.readFile(this.fullPath, 'utf-8');
    const section = (JSON.parse(sectionJson) as Array<any>).map(x => new app.api.models.SeriesListItem(x));
    await clv.validateOrReject(section);
    return section;
  }

  async saveAsync(section: Array<app.api.models.SeriesListItem>) {
    await clv.validateOrReject(section);

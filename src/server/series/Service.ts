...
export class Service {
  ...
  async scanRootAsync(sectionId: string, rootPaths: Array<string>) {
    await this.lockService.lockAsync(sectionId, 'series', async () => {
      const purgeAsync = this.cacheService.createPurgeable(
        `series.${sectionId}`
      );
      ...
      await Promise.all(
        rootPaths.map(async rootPath => {
          for await (const result of this.inspectRootAsync(rootPath)) {
            ...
            const seriesPrevious = await seriesCache
              .loadAsync()
              .catch(() => undefined);
  ...
  async scanSeriesAsync(sectionId: string, seriesId: string) {
    return await this.lockService.lockAsync(sectionId, undefined, async () => {
      ...
      if (seriesIndex !== -1) {
        ...
        const context = await this.contextService.contextAsync(
          path.dirname(series.path)
        );
        const seriesInfo = context.info.get('tvshow.nfo');
        if (seriesInfo) {
          const seriesUpdate = await this.inspectSeriesAsync(
            context,
            seriesInfo
          )
            .then(x => this.mergeSeriesAsync(x, series))
            .catch(() =>
              logger.error(`Invalid series: ${seriesInfo.fullPath}`)
            );
          if (seriesUpdate) {
            section[seriesIndex] =
              app.api.models.SeriesEntry.from(seriesUpdate);
            await Promise.all([
              sectionCache.saveAsync(section),
              seriesCache.saveAsync(seriesUpdate)
            ]);
            await this.eventService.sendAsync('series', sectionId);
            return true;
          }
  ...
  async patchAsync(
    sectionId: string,
    seriesId: string,
    seriesPatch: app.api.models.SeriesPatch
  ) {
    ...
    return await this.lockService.lockAsync(sectionId, undefined, async () => {
      ...
      if (seriesIndex !== -1) {
        const seriesCache = new SeriesCache(sectionId, seriesId);
        const series = await seriesCache.loadAsync();
        const episodeUpdates = Array.from(
          this.patchSeriesEpisodes(series, seriesPatch, now)
        );
        ...
        await Promise.all(
          episodeUpdates.map(x => EpisodeInfo.saveAsync(x.path, x))
        );
  ...
  private async *inspectRootAsync(rootPath: string) {
    ...
    for await (const context of subdirContexts) {
      const seriesInfo = context.info.get('tvshow.nfo');
      if (seriesInfo) {
        const series = await this.inspectSeriesAsync(context, seriesInfo).catch(
          () => logger.error(`Invalid series: ${seriesInfo.fullPath}`)
        );
  ...
  private async inspectSeriesAsync(
    context: Awaited<ReturnType<app.core.ContextService['contextAsync']>>,
    seriesStats: fs.Stats & {fullPath: string}
  ) {
    const seriesInfo = await SeriesInfo.loadAsync(seriesStats.fullPath);
    const rootEpisodes = new app.Linq(context.info.entries())
      .filter(([x]) => x !== 'tvshow.nfo')
      .map(([_, x]) =>
        this.inspectEpisodeAsync(context, x).catch(() =>
          logger.warn(`Invalid episode: ${x.fullPath}`)
        )
      );
    const subdirEpisodes = new app.Linq(context.directories.values())
      .map(x => this.contextService.contextAsync(x.fullPath))
      .flatMap(context =>
        new app.Linq(context.info.values()).map(x => ({context, ...x}))
      )
      .map(x =>
        this.inspectEpisodeAsync(x.context, x).catch(() =>
          logger.warn(`Invalid episode: ${x.fullPath}`)
        )
      );
    const episodes = (
      await rootEpisodes.concat(subdirEpisodes).toArrayAsync()
    ).sort((a, b) => a.season - b.season || a.episode - b.episode);
    const images = await new app.Linq(context.images.entries())
      .filter(([_, x]) =>
        episodes.every(y => !y.media.images?.some(z => z.path === x.fullPath))
      )
      .map(
        ([_, x]) =>
          new app.api.models.Media({
            id: app.id(`${x.fullPath}/${x.mtimeMs}`),
            path: x.fullPath
          })
      )
      .toArrayAsync();
    return new app.api.models.Series({
      ...
      dateAdded:
        seriesInfo.dateAdded ??
        (await this.timeService.getAsync(
          new app.Linq(episodes)
  ...
  private async inspectEpisodeAsync(
    context: Awaited<ReturnType<app.core.ContextService['contextAsync']>>,
    episodeStats: fs.Stats & {fullPath: string}
  ) {
    const {name} = path.parse(episodeStats.fullPath);
    const episodeInfo = await EpisodeInfo.loadAsync(episodeStats.fullPath);
    const images = await new app.Linq(context.images.entries())
    ...
    return new app.api.models.Episode({
      ...
      dateAdded:
        episodeInfo.dateAdded ??
        (await this.timeService.getAsync(
          new app.Linq(context.videos.entries())
            .filter(([x]) => x.startsWith(`${name}.`))
  ...
  private async mergeSeriesAsync(
    series: app.api.models.Series,
    previous?: app.api.models.Series
  ) {
    const episodes = await Promise.all(
      series.episodes.map(async x => {
        await this.mergeEpisodeAsync(
          x,
          previous?.episodes.find(y => x.id == y.id)
        );
        return x;
      })
    );
  ...
  private async mergeEpisodeAsync(
    episode: app.api.models.Episode,
    previous?: app.api.models.Episode
  ) {
    if (
      (typeof episode.lastPlayed === 'undefined' && previous?.lastPlayed) ||
      ...
      return await EpisodeInfo.saveAsync(
        episode.path,
        new app.api.models.Episode({
          ...episode,
  ...
  private patchSeries(
    series: app.api.models.Series,
    episodeUpdates: Array<app.api.models.Episode>,
    now: string
  ) {
    return new app.api.models.Series({
      ...series,
      lastPlayed: episodeUpdates.some(x => x.watched || x.resume)
        ? now
        : series.lastPlayed,
  ...
  private *patchSeriesEpisodes(
    ...
    for (const episode of seriesPatch.episodes) {
      ...
      if (index === -1) continue;
      series.episodes[index] = this.patchEpisode(
  ...
  private patchEpisode(
    episode: app.api.models.Episode,
    episodePatch: app.api.models.EpisodePatch,
    now: string
  ) {
    return new app.api.models.Episode({
      ...episode,
      ...episodePatch,
      lastPlayed:
        episodePatch.watched || episodePatch.resume ? now : episode.lastPlayed,
      playCount: episodePatch.watched
        ? (episode.playCount ?? 0) + 1
        : episode.playCount
    });
  }
}

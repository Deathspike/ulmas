...
export class Service {
      ...
      await Promise.all(
        rootPaths.map(async rootPath => {
          for await (const result of this.inspectRootAsync(rootPath)) {
            ...
            const moviePrevious = await movieCache
              .loadAsync()
              .catch(() => undefined);
    ...
    return await this.lockService.lockAsync(sectionId, undefined, async () => {
      ...
      if (movieIndex !== -1) {
        ...
        const context = await this.contextService.contextAsync(
          path.dirname(movie.path)
        );
        const movieInfo = context.info.get(path.basename(movie.path));
        if (movieInfo) {
          const movieUpdate = await this.inspectMovieAsync(context, movieInfo)
          ...
          if (movieUpdate) {
            ...
            await Promise.all([
              ...
              movieCache.saveAsync(movieUpdate)
            ]);
            await this.eventService.sendAsync('movies', sectionId);
            return true;
          }
  ...
  async patchAsync(
    sectionId: string,
    movieId: string,
    moviePatch: app.api.models.MoviePatch
  ) {
  ...
  private async *inspectRootAsync(rootPath: string) {
    ...
    const contextMovies = new app.Linq(context.info.values()).map(x => ({
      context,
      ...x
    }));
  ...
  private async inspectMovieAsync(
    context: Awaited<ReturnType<app.core.ContextService['contextAsync']>>,
    movieStats: fs.Stats & {fullPath: string}
  ) {
    const {name} = path.parse(movieStats.fullPath);
    const movieInfo = await MovieInfo.loadAsync(movieStats.fullPath);
    const hasPrivateRoot = await new app.Linq(context.info.values()).everyAsync(
      x => x.fullPath === movieStats.fullPath
    );
  ...
  private async mergeAsync(
    movie: app.api.models.Movie,
    previous?: app.api.models.Movie
  ) {
    if (
      (typeof movie.lastPlayed === 'undefined' && previous?.lastPlayed) ||
      ...
      return await MovieInfo.saveAsync(
        movie.path,
        new app.api.models.Movie({
          ...movie,
  ...
  private patchMovie(
    ...
    return new app.api.models.Movie({
      ...
      ...moviePatch,
      lastPlayed:
        moviePatch.watched || moviePatch.resume ? now : movie.lastPlayed,
      playCount: moviePatch.watched
        ? (movie.playCount ?? 0) + 1
        : movie.playCount
    });
  }
